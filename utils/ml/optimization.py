...
def sample_then_minimize_with_constraints(
        optimiser_func: Callable,
        bounds: np.ndarray,
        unconstrained_dims: [],
        constrained_vals: [],
    ...
    # remove constrained dims for generating samples for x
    unconstrained_bounds = bounds[unconstrained_dims]
    x_constrained_samples = np.tile(constrained_vals, [num_samples, 1])
    x_unconstrained_samples = np.random.uniform(unconstrained_bounds[:, 0],
                                                unconstrained_bounds[:, 1],
                                                (num_samples, unconstrained_bounds.shape[0]))


    # append latest constrained value to samples generated
    x_samples = np.concatenate([x_constrained_samples, x_unconstrained_samples], axis=-1)
